...
class cinta():
    ...
    def __str__(self):
        ...
        # if self.mixta:
        #     m = "Mixta"
        return "{};{};{};{};{};{}".format(self.Label,len(self.objetosdecinta),self.tamanos[0], self.tamanos[1], self.tamanos[2], self.tamanos[3])
#        return "{}".format(self.Label)
    ...
    def calculartamanos(self):
        """devuelve la lista [tamanopermanente, tamanoprotegido, tamanoexpirado, tamanootro]"""
        tamanopermanente = 0
        tamanoprotegido = 0
        tamanoexpirado = 0
        tamanootro = 0
        for i in self.objetosdecinta:
            ...
            if  "Permanent" in proteccion:
                tamanopermanente = tamanopermanente + i.tamano()
            elif ("Expired" in proteccion) or ("Not protected" in proteccion) or (proteccion == "None"):
                tamanoexpirado = tamanoexpirado + i.tamano()
            elif "Protected" in proteccion:
                tamanoprotegido = tamanoprotegido + i.tamano()
            else:
                tamanootro = tamanootro + i.tamano()
        return [tamanopermanente, tamanoprotegido, tamanoexpirado, tamanootro]
...
class objetodecinta:
    ...
    def  __str__(self):
        """print method"""
        return "{};{};{};{}".format(self.SessionID,self.Finished,self.Protection, self.Object_size)
    def tamano(self):
        """Tamano del objeto"""
    ...
    def calcularproteccion(self):
        ...
        if "Permanent" in self.Protection:
            ...
            return "Expired"
        elif "Protected" in self.Protection: #No quedan mas opciones, se trata de un contenido protegido
