...
#################################
def aux_antiguos(nombretemplate='antiguos.j2'):
    ...
    for r in roc:
        ...
        if (proteccion > 864000) or (contenidoCaliente(fechaExpiracion, fechaBackup)):
            ...
            tamanyo = int(r[3])
            cinta = r[4]
            serverSource = r[5].split(" '")[0]
            server, source = serverSource.split(":")

            index=len(ssa.keys()) + 1
            ssa[index]=[clave,tamanyo,cinta,server,source]
    ...
    return render_template(nombretemplate,sesionesSinAcumular=ssa,servidor=servidor,fecha=fecha)
